...
3. **Allowed actions で `npm publish` を有効にする**

   2026-09-03 以降に作成した設定は、既定で **`npm stage publish` のみ**が許可されます。
   本ワークフローは `npm publish` を直接実行するため、これを有効にしないと公開が拒否されます。
   （段階公開に切り替える場合は `release.yml` の publish ステップを `npm stage publish` に変更し、
   公開後に npm 側で promote する運用にしてください。）
...
- npm CLI **11.5.1 以上**（Node 22 同梱の npm では古いため、ワークフローで `npm@^11.5.1` へ更新しています）
...
| OIDCなのに publish が拒否される | Trusted Publisher の **Allowed actions** で `npm publish` が有効になっているか確認する（2026-09-03 以降に作成した設定は既定が `npm stage publish` のみ） |
